...
var ErrorInterceptor = /** @class */ (function () {
    ...
    //handles routes before and after request
    ErrorInterceptor.prototype.intercept = function (request, next) {
        var _this = this;
        return next.handle(request).pipe(rxjs_1.catchError(function (error) {
            if (error) {
                if (error.status === 400) {
                    _this.toastr.error(error.error.message, error.status.toString());
                }
                if (error.status === 401) {
                    _this.toastr.error(error.error.message, error.status.toString());
                }
            ...
            return rxjs_1.throwError(function () { return new Error(error.message); });
        }));
    };
